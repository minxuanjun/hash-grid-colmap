...
#include "cuda/utils/Comparators.cuh"
#include "cuda/utils/DeviceDefs.cuh"
#include "cuda/utils/MergeNetworkBlock.cuh"
#include "cuda/utils/MergeNetworkWarp.cuh"
#include "cuda/utils/PtxUtils.cuh"
#include "cuda/utils/ReductionOperators.cuh"
...
struct FinalBlockMerge<4, NumThreads, K, V, NumWarpQ, Dir, Comp> {
  static inline __device__ void merge(K* sharedK, V* sharedV) {
    blockMerge<NumThreads, K, V, NumThreads / (kWarpSize * 2), NumWarpQ, !Dir, Comp>(
        sharedK, sharedV);
    // Final merge doesn't need to fully merge the second list
...
struct FinalBlockMerge<8, NumThreads, K, V, NumWarpQ, Dir, Comp> {
  static inline __device__ void merge(K* sharedK, V* sharedV) {
    blockMerge<NumThreads, K, V, NumThreads / (kWarpSize * 2), NumWarpQ, !Dir, Comp>(
        sharedK, sharedV);
    blockMerge<NumThreads, K, V, NumThreads / (kWarpSize * 4), NumWarpQ * 2, !Dir, Comp>(
        sharedK, sharedV);
    // Final merge doesn't need to fully merge the second list
...
#pragma unroll
  ...
  __device__ inline void resetThreadQueue() {
#pragma unroll
    for (int i = 0; i < NumThreadQ; ++i) {
      threadK[i] = initK;
      threadV[i] = initV;
  ...
  __device__ inline void resetWarpRegisters() {
#pragma unroll
    for (int i = 0; i < kNumWarpQRegisters; ++i) {
      warpK[i] = initK;
      warpV[i] = initV;
