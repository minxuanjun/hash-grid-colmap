...
#include <cstring>
#include <limits>
#include "cuda/impl/GpuCannAssert.h"
...
template <
    ...
    template <typename U>
    class PtrTraits>
__host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::Tensor() : data_(nullptr) {
...
template <
    ...
    class PtrTraits>
__host__ __device__
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::Tensor(DataPtrType data, const IndexT sizes[Dim])
    : data_(data) {
  static_assert(Dim > 0, "must have > 0 dimensions");
...
__host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::Tensor(
    ...
    : data_(data) {
  GPU_CANN_ASSERT(sizes.size() == Dim);
  static_assert(Dim > 0, "must have > 0 dimensions");
...
__host__ void Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::copyFrom(
    const Tensor<T, Dim, InnerContig, IndexT, PtrTraits>& t, cudaStream_t stream) {
  // The tensor must be fully contiguous
  GPU_CANN_ASSERT(this->isContiguous());

  // Size must be the same (since dimensions are checked and
  // continuity is assumed, we need only check total number of
  // elements
  GPU_CANN_ASSERT(this->numElements() == t.numElements());

  if (t.numElements() > 0) {
    GPU_CANN_ASSERT(this->data_);
    GPU_CANN_ASSERT(t.data());
    ...
    if (tDev == -1) {
      ...
      CUDA_CHECK(cudaMemcpyAsync(
          ...
          t.data(),
          this->getSizeInBytes(),
          ourDev == -1 ? cudaMemcpyDeviceToHost : cudaMemcpyDeviceToDevice,
          stream));
    }
  }
}
  ...
  if (t.numElements() > 0) {
    ...
    if (tDev == -1) {
      CUDA_CHECK(cudaMemcpyAsync(
          ...
          this->data_,
          this->getSizeInBytes(),
          ourDev == -1 ? cudaMemcpyHostToHost : cudaMemcpyDeviceToHost,
          stream));
    } else {
      CUDA_CHECK(cudaMemcpyAsync(
          t.data(),
          this->data_,
          this->getSizeInBytes(),
          ourDev == -1 ? cudaMemcpyHostToDevice : cudaMemcpyDeviceToDevice,
          stream));
    }
  }
}
...
template <
    ...
    class PtrTraits>
__host__ void Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::copyFrom(
    const std::vector<T>& v, cudaStream_t stream) {
  // The tensor must be fully contiguous
  GPU_CANN_ASSERT(this->isContiguous());

  // Size must be the same
  GPU_CANN_ASSERT(this->numElements() == v.size());
...
template <
    ...
    template <typename U>
    class PtrTraits>
__host__ std::vector<T> Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::copyToVector(
    cudaStream_t stream) {
  // The tensor must be fully contiguous
  GPU_CANN_ASSERT(this->isContiguous());
...
template <
    ...
    class PtrTraits>
template <typename OtherT, int OtherDim>
__host__ __device__ bool Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::isSameSize(
    const Tensor<OtherT, OtherDim, InnerContig, IndexT, PtrTraits>& rhs) const {
  if (Dim != OtherDim) {
    return false;
  }
...
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::castResize() {
  ...
  GPU_CANN_ASSERT(canCastResize<U>());
...
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::castResize() const {
  return const_cast<Tensor<T, Dim, InnerContig, IndexT, PtrTraits>*>(this)->castResize<U>();
}
...
template <
    ...
    class PtrTraits>
template <typename U>
__host__ __device__ bool Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::canCastResize() const {
  static_assert(sizeof(U) >= sizeof(T), "only handles greater sizes");
  constexpr int kMultiple = sizeof(U) / sizeof(T);
...
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::castIndexType() const {
  if (sizeof(NewIndexT) < sizeof(IndexT)) {
    GPU_CANN_ASSERT(this->canUseIndexType<NewIndexT>());
  }
  ...
  return Tensor<T, Dim, InnerContig, NewIndexT, PtrTraits>(data_, newSize, newStride);
}
...
template <
    ...
    class PtrTraits>
template <typename NewIndexT>
__host__ bool Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::canUseIndexType() const {
...
template <
    ...
    template <typename U>
    class PtrTraits>
__host__ __device__ IndexT Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::numElements() const {
  auto size = getSize(0);
...
template <
    ...
    template <typename U>
    class PtrTraits>
__host__ __device__ bool Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::isContiguous() const {
  IndexT prevSize = 1;
...
template <
    ...
    template <typename U>
    class PtrTraits>
__host__ __device__ bool Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::isConsistentlySized(
    int i) const {
...
template <
    ...
    template <typename U>
    class PtrTraits>
__host__ __device__ bool Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::isContiguousDim(
    int i) const {
...
template <
    ...
    class PtrTraits>
__host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits>
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::transpose(int dim1, int dim2) const {
  GPU_CANN_ASSERT(dim1 >= 0 && dim1 < Dim);
  GPU_CANN_ASSERT(dim2 >= 0 && dim2 < Dim);

  // If a tensor is innermost contiguous, one cannot transpose the innermost
  // dimension
  if (InnerContig) {
    GPU_CANN_ASSERT(dim1 != Dim - 1 && dim2 != Dim - 1);
  }
...
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::transposeInnermost(int dim1) const {
  GPU_CANN_ASSERT(dim1 >= 0 && dim1 < Dim);
...
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::upcastOuter() {
  ...
  return Tensor<T, NewDim, InnerContig, IndexT, PtrTraits>(data_, newSize, newStride);
}
...
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::downcastInner() {
  ...
  for (int i = NewDim; i < Dim; ++i) {
    GPU_CANN_ASSERT(isContiguousDim(i));
  }
...
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::view(DataPtrType at) {
  static_assert(SubDim >= 1 && SubDim < Dim, "can only create view of lesser dim");
  ...
  return Tensor<T, SubDim, InnerContig, IndexT, PtrTraits>(at, viewSizes, viewStrides);
}
...
template <
    ...
    class PtrTraits>
__host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits>
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::narrowOutermost(IndexT start, IndexT size) {
...
template <
    ...
    class PtrTraits>
__host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits>
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::narrow(int dim, IndexT start, IndexT size) {
  DataPtrType newData = data_;

  GPU_CANN_ASSERT(start >= 0 && start < size_[dim] && (start + size) <= size_[dim]);
...
template <int NewDim>
__host__ __device__ Tensor<T, NewDim, InnerContig, IndexT, PtrTraits>
Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::view(std::initializer_list<IndexT> sizes) {
  GPU_CANN_ASSERT(this->isContiguous());

  GPU_CANN_ASSERT(sizes.size() == NewDim);
  ...
  GPU_CANN_ASSERT(curSize == newSize);
