...
#include <vector>
#include "cuda/MetricType.h" // idx_t
...
class Tensor {
 public:
  ...
  using TensorType = Tensor<T, Dim, InnerContig, IndexT, PtrTraits>;
  /// Default constructor
  ...
  /// Assignment
  __host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits>& operator=(
  ...
  template <typename U>
  __host__ __device__ const Tensor<U, Dim, InnerContig, IndexT, PtrTraits> castResize() const;
  ...
  /// Cast to a different datatype
  template <typename U>
  __host__ __device__ inline const typename PtrTraits<const U>::PtrType dataAs() const {
    return reinterpret_cast<typename PtrTraits<const U>::PtrType>(data_);
  }

  /// Returns a read/write view of a portion of our tensor.
  __host__ __device__ inline detail::SubTensor<TensorType, Dim - 1, PtrTraits> operator[](IndexT);

  /// Returns a read/write view of a portion of our tensor (const).
  __host__ __device__ inline const detail::SubTensor<TensorType, Dim - 1, PtrTraits> operator[](
      IndexT) const;
  ...
  __host__ __device__ Tensor<T, Dim, false, IndexT, PtrTraits> transposeInnermost(int dim1) const;
  ...
  __host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits> narrowOutermost(
      IndexT start, IndexT size);
...
bool canUseIndexType(const T& arg, const U&... args) {
  return arg.template canUseIndexType<IndexType>() && canUseIndexType(args...);
}
...
class SubTensor<TensorType, 0, PtrTraits> {
 public:
  ...
  __host__ __device__ inline const typename TensorType::DataPtrType data() const {
    return data_;
  }
  ...
  /// Cast to a different datatype (const)
  template <typename T>
  __host__ __device__ inline typename PtrTraits<const T>::PtrType dataAs() const {
 ...
 protected:
  ...
  __host__ __device__ inline SubTensor(TensorType& t, typename TensorType::DataPtrType data)
...
class SubTensor {
 public:
  ...
  /// `SubDim` - 1 tensor).
  __host__ __device__ inline SubTensor<TensorType, SubDim - 1, PtrTraits> operator[](
    ...
    if (TensorType::IsInnerContig && SubDim == 1) {
      ...
      return SubTensor<TensorType, SubDim - 1, PtrTraits>(tensor_, data_ + index);
    } else {
      return SubTensor<TensorType, SubDim - 1, PtrTraits>(
          tensor_, data_ + index * tensor_.getStride(TensorType::NumDim - SubDim));
    }
  }

  /// Returns a view of the data located at our offset (the dimension
  /// `SubDim` - 1 tensor) (const).
  __host__ __device__ inline const SubTensor<TensorType, SubDim - 1, PtrTraits> operator[](
...
__host__ __device__ inline detail::
    SubTensor<Tensor<T, Dim, InnerContig, IndexT, PtrTraits>, Dim - 1, PtrTraits>
...
__host__ __device__ inline const detail::
  ...
  return detail::SubTensor<TensorType, Dim - 1, PtrTraits>(
      detail::SubTensor<TensorType, Dim, PtrTraits>(const_cast<TensorType&>(*this), data_)[index]);
}

} // namespace gpu
} // namespace faiss

#include <cuda/utils/Tensor-inl.cuh>
